...
<?php
$metaTitle = isset($artikel['title']) ? $artikel['title'] : 'Detail Artikel';
$rawDesc = isset($artikel['content']) ? strip_tags($artikel['content']) : '';
$metaDescription = trim(mb_substr(preg_replace('/\s+/', ' ', $rawDesc), 0, 160));
$ogImage = !empty($artikel['image']) ? base_url($artikel['image']) : base_url('landingpage/assets/img/blog/blog-post-1.webp');
$canonical = site_url('artikel/' . ($artikel['slug'] ?? ''));
?>
        ...
        <div class="col-lg-8">
            <article class="card shadow-sm mb-4">
                <?php if (!empty($artikel['image'])): ?>
                    <img src="<?= base_url($artikel['image']) ?>" class="card-img-top img-fluid"
                        alt="<?= esc($artikel['title']) ?>">
                <?php endif; ?>
                <div class="card-body">
                    <h1 class="card-title mb-2"><?= esc($artikel['title']) ?></h1>
                    ...
                    <div class="mt-4 d-flex gap-2">
                        <a href="/kelola-artikel" class="btn btn-secondary">Kembali</a>
                        <?php if (session()->get('role') === 'admin' || session()->get('username') === $artikel['author']): ?>
                            <a href="<?= site_url('kelola-artikel/' . $artikel['id_artikel'] . '/edit') ?>"
                                class="btn btn-primary">Edit</a>
                            <form method="post" action="<?= site_url('kelola-artikel/' . $artikel['id_artikel']) ?>"

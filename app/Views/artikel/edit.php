...
<div class="container py-4">
    <form action="<?= site_url('kelola-artikel/' . $artikel['id_artikel']) ?>" method="post"
        enctype="multipart/form-data" class="card p-4">
        <?= csrf_field() ?>
        <input type="hidden" name="_method" value="PUT">
        <div class="mb-3">
            <label for="title" class="form-label">Judul</label>
            <input type="text" name="title" id="title" class="form-control" value="<?= esc($artikel['title']) ?>"
                required>
        </div>

        <div class="mb-3">
            <label for="slug" class="form-label">Slug (otomatis)</label>
            <input type="text" id="slug" class="form-control" readonly value="<?= esc($artikel['slug']) ?>">
        </div>

        <div class="mb-3">
            <label for="content" class="form-label">Konten</label>
            <textarea name="content" id="content" rows="10"
                class="form-control d-none"><?= old('content', $artikel['content']) ?></textarea>
            <div id="quillEditor" style="height: 400px;"><?= old('content', $artikel['content']) ?></div>
        </div>
        ...
        <div class="mb-3">
            ...
            <?php if (!empty($artikel['image'])): ?>
                <img id="imgPreview" src="<?= base_url($artikel['image']) ?>" alt="Preview gambar"
        ...
        <div class="mb-3">
            ...
            <input type="text" name="source" id="source" class="form-control"
                value="<?= old('source', $artikel['source']) ?>">
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="date" name="tanggal" id="tanggal" class="form-control"
                    value="<?= old('tanggal', date('Y-m-d', strtotime($artikel['tanggal']))) ?>">
            </div>
        </div>
